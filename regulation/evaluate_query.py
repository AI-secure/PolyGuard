...
def evaluate_guardrails_query():
    """Evaluate guardrail models on query prompts (safe and unsafe) and save results."""
    parser = argparse.ArgumentParser(description="Evaluate guardrail models on GuardBench query dataset")
    parser.add_argument("--model_id", "-m", type=str, required=True, help="Guardrail model ID to evaluate")
    args = parser.parse_args()

    # Paths
    base_dir = "./"
    dataset_dir = f"{base_dir}/dataset/regulation/eu_ai_act/query"
    ...
    output_dir = os.path.join(output_base_dir, model_name, "query")
    ...
    unsafe_output_file = os.path.join(output_dir, "unsafe_result.jsonl")
    ...
    with open(output_file, 'w', encoding='utf-8') as out_f:
        for prompt_data in tqdm(prompts, desc=f"Evaluating {true_label} prompts"):
            ...
            try:
                ...
                moderation_result = model.moderate(prompt_text)
                ...
                # Add original fields from the prompt data except "prompt" which is already included
                original_data = {k: v for k, v in prompt_data.items() if k != "prompt"}
                ...
                print(f"Error evaluating prompt '{prompt_text[:50]}...': {e}")
                continue

if __name__ == "__main__":
    evaluate_guardrails_query() 